...
For example, if your data contains attributes named C<mother> and
C<father>, and each points to a C<person_id>, you could specify
L</alias_attributes> as

  {  person_id => [ 'mother', 'father' ] }

and both would be replaced using the same remapping as C<person_id>,
so they would still point to the correct person.

Note that in the base implementation, this type of aliasing is only
applied in L</remap_id> and L</remap_label>, since attribute name does
not affect remapping of dates.  Theoretically, a subclass might
implement a date mapping that B<does> depend on the attribute name, in
which case it must respect aliases.

=cut

has 'alias_attributes' =>
  ( isa => Maybe[HashRef], is => 'ro', required => 0, lazy => 1,
    builder => 'build_alias_attributes' );

sub build_alias_attributes {}
...
=item remap_id($record, $key, $options)

Return a numeric substitute value for the contents of I<<
$record->{$key} >>.  In list context, returns the original value,
followed by the substitute.  If the original value is C<undef> or an
empty string, then it is returned unchanged.

If I<$options> is present, it must be a hash reference.  The key
C<map_name>, if present, specifies a name to use rather than I<$key>
...
just lets you have values for different I<$key>s be replaced using the
same map.
...
sub remap_id {
  my($self, $rec, $key, $options) = @_;
  $options //= {};
  my $orig = $rec->{$key};
  return $orig unless defined $orig and length $orig;

  my $map = $self->_id_map;

  # From this point onward, we use $key only to work with the map, not
  # with $rec, so we can safely replace its value
  $key = $options->{map_name} // $key;
  $map->{$key} //= {};
...
=item remap_label($rec, $key, $options)
...
sub remap_label {
 my($self, $rec, $key, $options) = @_; 
 return $rec->{$key} unless defined $rec->{$key} and length $rec->{$key};
 my $new = sprintf('%s_%d', $key, scalar $self->remap_id($rec, $key, $options));
...
sub scrub_record {
  ...
  foreach my $k (keys %new) {
    ...
    if (keys %$aliases and $k ~~ [ values %$aliases ]) {
      foreach my $alias (keys %$aliases) {
	if ($k ~~ $aliases->{$alias}) {
	  $options->{map_name} = $alias;
	  last;
	}
      }
    }
